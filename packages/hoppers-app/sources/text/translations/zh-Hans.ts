...
export const zhHans: TranslationStructure = {
    ...
    settings: {
        ...
        about: '关于',
        aboutFooter: 'Hoppers 是一个 Codex 和 Claude Code 移动客户端。它采用端到端加密，您的账户仅存储在本地设备上。与 Anthropic 无关联。',
        whatsNew: '更新日志',
    ...
    sessionInfo: {
        ...
        updateCliInstructions: '请运行 npm install -g hoppers@latest',
        deleteSession: '删除会话',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有终端正在请求连接到您的 Hoppers 账户。这将允许终端安全地发送和接收消息。',
    ...
    machine: {
        ...
        offlineHelp: '• 确保您的计算机在线\n• 运行 `hoppers daemon status` 进行诊断\n• 您是否在运行最新的 CLI 版本？请使用 `npm install -g hoppers@latest` 升级',
        daemon: '守护进程',
        status: '状态',
        stopDaemon: '停止守护进程',
