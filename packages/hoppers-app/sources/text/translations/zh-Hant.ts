...
export const zhHant: TranslationStructure = {
    ...
    settings: {
        ...
        about: '關於',
        aboutFooter: 'Hoppers 是一個 Codex 和 Claude Code 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。',
        whatsNew: '更新日誌',
    ...
    sessionInfo: {
        ...
        updateCliInstructions: '請執行 npm install -g hoppers@latest',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有終端正在請求連線到您的 Hoppers 帳戶。這將允許終端安全地傳送和接收訊息。',
    ...
    machine: {
        ...
        offlineHelp: '• 確保您的電腦在線上\n• 執行 `hoppers daemon status` 進行診斷\n• 您是否在執行最新的 CLI 版本？請使用 `npm install -g hoppers@latest` 升級',
        daemon: '守護程序',
        status: '狀態',
        stopDaemon: '停止守護程序',
