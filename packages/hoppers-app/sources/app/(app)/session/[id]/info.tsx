...
import { HoppersError } from '@/utils/errors';
...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const [archivingSession, performArchive] = useHoppersAction(async () => {
        const result = await sessionKill(session.id);
        if (!result.success) {
            throw new HoppersError(result.message || t('sessionInfo.failedToArchiveSession'), false);
        }
    ...
    const [deletingSession, performDelete] = useHoppersAction(async () => {
        const result = await sessionDelete(session.id);
        if (!result.success) {
            throw new HoppersError(result.message || t('sessionInfo.failedToDeleteSession'), false);
        }
    ...
    const handleCopyUpdateCommand = useCallback(async () => {
        const updateCommand = 'npm install -g hoppers@latest';
        try {
