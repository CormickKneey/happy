...
import { useHoppersAction } from '@/hooks/useHoppersAction';
...
export const SettingsView = React.memo(function SettingsView() {
    ...
    const handleGitHub = async () => {
        const url = 'https://github.com/CormickKneey/hoppers';
    ...
    const handleReportIssue = async () => {
        const url = 'https://github.com/CormickKneey/hoppers/issues';
        const supported = await Linking.canOpenURL(url);
        if (supported) {
            await Linking.openURL(url);
        }
    ...
    // GitHub connection
    const [connectingGitHub, connectGitHub] = useHoppersAction(async () => {
        const params = await getGitHubOAuthParams(auth.credentials!);
        await Linking.openURL(params.url);
    });

    // GitHub disconnection
    const [disconnectingGitHub, handleDisconnectGitHub] = useHoppersAction(async () => {
    ...
    const [connectingAnthropic, connectAnthropic] = useHoppersAction(async () => {
        router.push('/settings/connect/claude');
    });

    // Anthropic disconnection
    const [disconnectingAnthropic, handleDisconnectAnthropic] = useHoppersAction(async () => {
        const confirmed = await Modal.confirm(
        ...
        <ItemList style={{ paddingTop: 0 }}>
            ...
            <ItemGroup title={t('settings.about')} footer={t('settings.aboutFooter')}>
                ...
                <Item
                    ...
                    onPress={async () => {
                        const url = 'https://github.com/CormickKneey/hoppers/blob/main/TERMS.md';
                        const supported = await Linking.canOpenURL(url);
                        if (supported) {
                            await Linking.openURL(url);
                        }
