...
This document describes the shared wire package: `@hoppers-app/hoppers-wire`.

## Why this package exists

Before `hoppers-wire`, wire-level message and session-protocol schemas were duplicated across packages (CLI, app, server, and agent). That caused drift risk and made protocol evolution harder.

`@hoppers-app/hoppers-wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@hoppers-app/hoppers-wire`
- workspace path: `packages/hoppers-wire`
...
Shared from `@hoppers-app/hoppers-wire`:
...
### CLI (`packages/hoppers-cli`)

- Session protocol imports now reference `@hoppers-app/hoppers-wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@hoppers-app/hoppers-wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@hoppers-app/hoppers-wire`.

### App (`packages/hoppers-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@hoppers-app/hoppers-wire`:
  - `ApiMessageSchema`
  - `ApiUpdateNewMessageSchema`
  - `ApiUpdateSessionStateSchema`
  - `ApiUpdateMachineStateSchema`

### Server (`packages/hoppers-server`)

- Prisma JSON message content type now references `SessionMessageContent` from `@hoppers-app/hoppers-wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/hoppers-agent`)

- `RawMessage` now aliases `SessionMessage` from `@hoppers-app/hoppers-wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@hoppers-app/hoppers-wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@hoppers-app/hoppers-wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn workspace @hoppers-app/hoppers-wire release
```

When building workspaces from a clean checkout, build `@hoppers-app/hoppers-wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@hoppers-app/hoppers-wire`.
4. Update downstream package versions if needed.
5. Publish dependent package updates only after the new `hoppers-wire` version is available.

## Notes

- `hoppers-wire` should stay focused on wire contracts only (types + Zod schemas + small helpers).
