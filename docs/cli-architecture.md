...
This document describes the Hoppers CLI (`packages/hoppers-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.

## System overview

```mermaid
graph TB
    subgraph "Hoppers CLI"
        Entry[src/index.ts]
        API[API Client]
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.hoppers`.
...
```mermaid
flowchart TD
    Start([hoppers ...]) --> Parse[Parse subcommand]
...
graph LR
    ...
    subgraph "Environment Overrides"
        direction TB
        E1[HOPPERS_HOME_DIR]
        E2[HOPPERS_SERVER_URL]
        E3[HOPPERS_WEBAPP_URL]
        E4[HOPPERS_VARIANT]
        E5[HOPPERS_EXPERIMENTAL]
        E6[HOPPERS_DISABLE_CAFFEINATE]
    end

    E1 -.-> settings & access & daemon & logs
```

Local state lives under `~/.hoppers` (or `HOPPERS_HOME_DIR`):
...
- `HOPPERS_SERVER_URL` and `HOPPERS_WEBAPP_URL` override defaults.
- `HOPPERS_VARIANT`, `HOPPERS_EXPERIMENTAL`, `HOPPERS_DISABLE_CAFFEINATE` control behavior.
...
- CLI entry: `packages/hoppers-cli/src/index.ts`
- Daemon: `packages/hoppers-cli/src/daemon`
- Control server/client: `packages/hoppers-cli/src/daemon/controlServer.ts`, `packages/hoppers-cli/src/daemon/controlClient.ts`
- API clients: `packages/hoppers-cli/src/api`
- Persistence: `packages/hoppers-cli/src/persistence.ts`
- Config: `packages/hoppers-cli/src/configuration.ts`
