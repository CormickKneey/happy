...
Implement `docs/session-protocol.md` as the new message format for Codex sessions in the CLI, and add client-side support in `hoppers-app` to parse, normalize, and render these messages alongside existing legacy formats (output, codex, acp).
...
- ⚠️ `packages/hoppers-cli` and `packages/hoppers-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/hoppers-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/hoppers-cli/src/api/apiSession.ts`
...
- [x] Run full test suite — `yarn test` in hoppers-cli, hoppers-app
