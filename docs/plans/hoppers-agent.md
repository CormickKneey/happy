# hoppers-agent CLI Tool

## Overview
A new standalone CLI tool (`hoppers-agent`) in `packages/hoppers-agent` that acts as a dedicated client for controlling Hoppers agents remotely. Unlike `hoppers-cli` which both runs and controls agents, `hoppers-agent` only controls them — creating sessions, sending messages, reading history, monitoring state, and stopping sessions.

This is a completely separate client from `hoppers-cli`. It has its own authentication flow (account auth via QR code, same as device linking in the mobile app), its own credential storage (`~/.hoppers/agent.key`), and is written from scratch with no code sharing.

## Context
- **Existing system**: Monorepo with `hoppers-cli` (agent runtime + control), `hoppers-server` (Fastify + PostgreSQL + Redis), `hoppers-app` (React Native mobile)
- **Server API**: REST endpoints at `https://api.cluster-fluster.com` + Socket.IO at `/v1/updates`
- **Authentication**: Uses account auth flow (`/v1/auth/account/request` + `/v1/auth/account/response`) — generates ephemeral keypair, displays QR code (`hoppers:///account?[base64url-publicKey]`), user scans with existing Hoppers mobile app to approve, receives encrypted account secret
- **Credential storage**: `~/.hoppers/agent.key` (separate from hoppers-cli's `~/.hoppers/access.key`)
- **Encryption**: AES-256-GCM (dataKey) for all new sessions. The master content keypair is derived deterministically from the account secret via `deriveKey(secret, 'Hoppers EnCoder', ['content'])` → seed → `crypto_box_seed_keypair(seed)`. Per-session random keys are encrypted with the master public key and stored on the server.
...
- [x] Create `packages/hoppers-agent/` directory with `package.json` (name: `hoppers-agent`, type: module, bin: `./bin/hoppers-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/hoppers-agent.mjs` entry point wrapper (mirrors hoppers-cli pattern: spawns node with `--no-warnings`)
...
- [x] Implement key derivation tree:
  ...
  - `deriveContentKeyPair(secret)` — calls `deriveKey(secret, 'Hoppers EnCoder', ['content'])` → seed → `sha512(seed)[0:32]` → `tweetnacl.box.keyPair.fromSecretKey()` → returns `{ publicKey, secretKey }`
...
- [x] Create `src/config.ts` — reads `HOPPERS_SERVER_URL` (default: `https://api.cluster-fluster.com`), `HOPPERS_HOME_DIR` (default: `~/.hoppers`), derives credential file path as `${hoppersHomeDir}/agent.key`
- [x] Create `src/credentials.ts`:
  ...
  - `readCredentials(config)` — parses `~/.hoppers/agent.key` JSON `{ token, secret }`, decodes secret from base64, derives contentKeyPair via `deriveContentKeyPair(secret)`. Returns `Credentials` or `null` if file missing.
  - `writeCredentials(config, token, secret)` — writes `{ token, secret: base64(secret) }` to `~/.hoppers/agent.key`
  - `clearCredentials(config)` — deletes `~/.hoppers/agent.key`
  - `requireCredentials(config)` — calls `readCredentials`, throws with "Run `hoppers-agent auth login` first" if null
...
### Task 4: Authentication command (`hoppers-agent auth`)
- [x] Create `src/auth.ts` implementing the account auth flow:
  ...
  3. Generate QR code data: `hoppers:///account?` + base64url(keypair.publicKey)
  4. Display QR code in terminal using `qrcode-terminal`
  5. Print instructions: "Scan this QR code with the Hoppers app (Settings → Account → Link New Device)"
...
- [x] Add `hoppers-agent auth login` subcommand that runs the flow above
- [x] Add `hoppers-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `hoppers-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `hoppers-agent`
- [x] `hoppers-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `hoppers-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `hoppers-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `hoppers-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `hoppers-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `hoppers-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `hoppers-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by hoppers-agent is visible and controllable from mobile app
- [x] Verify interop: session created by hoppers-cli can be listed and history read by hoppers-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/hoppers-agent/` with usage examples for all commands
...
```
hoppers-agent auth login                          # Authenticate via QR code (scanned by Hoppers mobile app)
hoppers-agent auth logout                         # Clear stored credentials
hoppers-agent auth status                         # Show authentication status

hoppers-agent list [--active] [--json]            # List all sessions
hoppers-agent status <session-id> [--json]        # Get live session state
hoppers-agent create --tag <tag> [--path <path>] [--json]  # Create new session
hoppers-agent send <session-id> <message> [--wait] [--json]  # Send message
hoppers-agent history <session-id> [--limit <n>] [--json]    # Read message history
hoppers-agent stop <session-id>                   # Stop a session
hoppers-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```

### Authentication Flow (Account Auth)
```
hoppers-agent                          Hoppers Server                    Hoppers Mobile App
     |                                    |                               |
     ...
     +-- Save { token, secret }           |                               |
     |   to ~/.hoppers/agent.key            |                               |
     |                                    |                               |
     +-- Derive content keypair:          |                               |
     |   deriveKey(secret,                |                               |
     |     'Hoppers EnCoder', ['content'])  |                               |
     |   -> seed -> box keypair           |                               |
...
### Credential File Format (`~/.hoppers/agent.key`)
```json
{
...
```
secret (32 bytes)
  -> deriveKey(secret, 'Hoppers EnCoder', ['content'])
  -> seed (32 bytes)
...
**For new sessions (created by hoppers-agent):**
...
**For existing sessions (created by hoppers-cli or other clients):**
...
## Post-Completion
**Manual verification:**
- Test full auth flow: run `hoppers-agent auth login`, scan QR with Hoppers app, verify credentials saved
...
- Test `history` command for sessions created by both `hoppers-agent` and `hoppers-cli`
- Test cross-client interop: messages from hoppers-agent readable by mobile app and vice versa

**Distribution:**
- Package can be published to npm as `hoppers-agent`
- Alternatively, users install from monorepo via `yarn workspace hoppers-agent build`
