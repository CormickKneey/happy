...
## Overview
Integrate `@anthropic-ai/sandbox-runtime` into hoppers-cli to sandbox both **Claude Code** and **Codex** sessions with OS-level filesystem and network restrictions. The sandbox wraps agent subprocesses, enforcing configurable restrictions without requiring containers.

Key features:
- **`hoppers sandbox configure`** - Interactive CLI wizard (using `inquirer`) to set up sandbox rules
- **`hoppers sandbox status`** - Show current sandbox configuration
- **`hoppers sandbox disable`** - Turn off sandboxing
- **Automatic enforcement** - Once configured, sandbox wraps both Claude and Codex sessions by default (bypass with `--no-sandbox`)
- **Global config** - Stored in `~/.hoppers/settings.json` alongside existing settings
- **Dual agent support** - Same sandbox config applies to both Claude Code and Codex

## Context
- **Claude spawn point**: `packages/hoppers-cli/src/claude/claudeLocal.ts:241` - `spawn('node', [claudeCliPath, ...args], {env, ...})`
- **Codex spawn point**: `packages/hoppers-cli/src/codex/codexMcpClient.ts:107` - `StdioClientTransport({ command: 'codex', args: ['mcp-server'], env })` which internally calls `cross-spawn('codex', ['mcp-server'])`
- **Config storage**: `packages/hoppers-cli/src/persistence.ts` - Settings interface + Zod schemas + atomic `updateSettings()`
- **Command dispatch**: `packages/hoppers-cli/src/index.ts` - manual `if/else if` routing on `args[0]`
- **Existing command pattern**: `packages/hoppers-cli/src/commands/connect.ts` - exported `handleXxxCommand(args)` functions
...
- [x] Run `yarn add @anthropic-ai/sandbox-runtime inquirer` in `packages/hoppers-cli`
- [x] Run `yarn add -D @types/inquirer` in `packages/hoppers-cli`
...
- [x] Create `packages/hoppers-cli/src/sandbox/config.ts`
...
- [x] Create `packages/hoppers-cli/src/sandbox/manager.ts`
...
### Task 5: Create `hoppers sandbox configure` interactive wizard
- [x] Create `packages/hoppers-cli/src/commands/sandbox.ts`
...
### Task 6: Implement `hoppers sandbox status` and `hoppers sandbox disable`
...
### Task 10: Add `hoppers sandbox` to help text and polish
- [x] Add `hoppers sandbox` to the help text in `index.ts` (alongside `auth`, `connect`, `daemon`, etc.)
...
- [x] Verify `hoppers sandbox configure` walks through all questions and saves config (automated command tests)
- [x] Verify `hoppers sandbox status` shows current config (automated command tests)
- [x] Verify `hoppers sandbox disable` turns off sandbox (automated command tests)
...
- ⚠️ Lint blocker: `packages/hoppers-cli` has no `eslint.config.*` / `.eslintrc*`, so ESLint 9 cannot run in this package.
...
**Manual verification:**
- Test `hoppers sandbox configure` end-to-end on macOS
