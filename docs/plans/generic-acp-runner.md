...
- **Session protocol types** in `@hoppers-app/hoppers-wire` (`SessionEnvelope`, `createEnvelope()`).
...
**File**: `packages/hoppers-cli/src/agent/acp/AcpSessionMapper.ts`
...
**Test file**: `packages/hoppers-cli/src/agent/acp/AcpSessionMapper.test.ts`
...
**File**: `packages/hoppers-cli/src/agent/acp/runAcp.ts`
...
2. Start Hoppers MCP server for tool bridge
...
Wire the generic runner into the CLI so users can run `hoppers acp gemini` or `hoppers acp opencode` or `hoppers acp -- custom-agent --flag`.

**Files**:
- `packages/hoppers-cli/src/index.ts` — add CLI command routing
...
- [ ] Add CLI routing for `hoppers acp <agent-name>` and `hoppers acp -- <cmd> [args]`
