...
⚠️ No lint script is defined in `packages/hoppers-server/package.json`, so linter execution is currently not available.
...
- Update `hoppers-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `hoppers-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
