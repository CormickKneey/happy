...
## Overview
Create a portable, self-contained distribution of hoppers-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `hoppers-server migrate` and `hoppers-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to hoppers-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile hoppers-server`
...
- [ ] Test: `./hoppers-server migrate` creates and migrates a PGlite database
- [ ] Test: `./hoppers-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./hoppers-server migrate` — verify database is created in `./data/`
- [ ] Run `./hoppers-server serve` — verify server starts, health endpoint responds
...
**CLI usage:**
```
hoppers-server migrate    # Apply database migrations
hoppers-server serve      # Start the server
```
...
- Consider adding `hoppers-server init` command for first-time setup
