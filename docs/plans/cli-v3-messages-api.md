# CLI V3 Messages API Migration (hoppers-cli)

## Overview
Migrate `hoppers-cli`'s `ApiSessionClient` from Socket.IO-based message read/write to the new v3 HTTP endpoints. The client will:
...
- **Package**: `packages/hoppers-cli/src/api/apiSession.ts` — `ApiSessionClient` class (EventEmitter)
...
- **InvalidateSync**: Available in `src/utils/sync.ts` (identical to hoppers-app's version)
...
- ⚠️ `npx eslint` in `packages/hoppers-cli` currently fails because there is no `eslint.config.(js|mjs|cjs)` in this workspace, so lint verification could not be completed.
