...
This document details how client data is encrypted, how encrypted blobs are structured, and how those blobs map onto protocol fields. It is based on `packages/hoppers-cli/src/api/encryption.ts` and the server routes that accept/emit these values.
...
These are the client-side structures that get encrypted and sent over the wire. They are defined in `packages/hoppers-cli/src/api/types.ts`.
...
- Client crypto: `packages/hoppers-cli/src/api/encryption.ts`
- Session message format: `packages/hoppers-cli/src/api/types.ts`
- Server message ingestion: `packages/hoppers-server/sources/app/api/socket/sessionUpdateHandler.ts`
- Artifact/KV routes: `packages/hoppers-server/sources/app/api/routes/artifactsRoutes.ts`, `packages/hoppers-server/sources/app/kv/kvMutate.ts`
