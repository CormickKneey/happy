...
This document describes the Hoppers backend structure as implemented in `packages/hoppers-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/hoppers-server/sources/main.ts`.
...
- API server: `packages/hoppers-server/sources/app/api/api.ts`
- Socket server: `packages/hoppers-server/sources/app/api/socket.ts`
- Event routing: `packages/hoppers-server/sources/app/events/eventRouter.ts`
- Presence: `packages/hoppers-server/sources/app/presence`
- Storage: `packages/hoppers-server/sources/storage`
- Prisma schema: `packages/hoppers-server/prisma/schema.prisma`
